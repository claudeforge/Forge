...
const statusConfig: Record<TaskDef["status"], { color: string; bgColor: string; label: string }> = {
  pending: { color: "text-gray-600 dark:text-gray-400", bgColor: "bg-gray-500/20", label: "Pending" },
  queued: { color: "text-blue-600 dark:text-blue-400", bgColor: "bg-blue-500/20", label: "Queued" },
  blocked: { color: "text-orange-600 dark:text-orange-400", bgColor: "bg-orange-500/20", label: "Blocked" },
  running: { color: "text-yellow-600 dark:text-yellow-400", bgColor: "bg-yellow-500/20", label: "Running" },
  completed: { color: "text-green-600 dark:text-green-400", bgColor: "bg-green-500/20", label: "Completed" },
  failed: { color: "text-red-600 dark:text-red-400", bgColor: "bg-red-500/20", label: "Failed" },
  skipped: { color: "text-gray-600 dark:text-gray-400", bgColor: "bg-gray-500/20", label: "Skipped" },
};
      ...
      >
        ...
        <div className="flex items-start justify-between p-4 border-b border-gray-200 dark:border-gray-700">
          <div className="flex-1 min-w-0 pr-4">
            ...
            <h2 className="text-xl font-semibold text-gray-900 dark:text-white">{taskDef.title}</h2>
            {taskDef.spec_id && (
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Spec: {taskDef.spec_id}</p>
            )}
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="bg-gray-100 dark:bg-gray-900 rounded-lg p-3">
              ...
              <p className="text-lg font-medium text-gray-900 dark:text-white">{taskDef.priority}</p>
            </div>
            <div className="bg-gray-100 dark:bg-gray-900 rounded-lg p-3">
              <div className="flex items-center gap-2 text-gray-500 dark:text-gray-400 text-xs mb-1">
              ...
              <p className="text-lg font-medium text-gray-900 dark:text-white">{taskDef.iterations}</p>
            </div>
            {taskDef.execution.max_iterations && (
              <div className="bg-gray-100 dark:bg-gray-900 rounded-lg p-3">
                ...
                <p className="text-lg font-medium text-gray-900 dark:text-white">{taskDef.execution.max_iterations}</p>
              </div>
            )}
            {taskDef.execution.timeout_minutes && (
              <div className="bg-gray-100 dark:bg-gray-900 rounded-lg p-3">
                ...
                <p className="text-lg font-medium text-gray-900 dark:text-white">{taskDef.execution.timeout_minutes}m</p>
              </div>
            )}
          </div>
          ...
          {(taskDef.depends_on.length > 0 || taskDef.blocks.length > 0) && (
            <div>
              ...
              <div className="bg-gray-100 dark:bg-gray-900 rounded-lg p-4 space-y-3 text-sm">
                {taskDef.depends_on.length > 0 && (
                  <div>
                    <span className="text-gray-500 dark:text-gray-400 block mb-1">Depends on:</span>
                    <div className="flex flex-wrap gap-1.5">
                      {taskDef.depends_on.map((dep) => (
                        <span key={dep} className="bg-gray-200 dark:bg-gray-800 text-gray-600 dark:text-gray-300 px-2 py-1 rounded font-mono text-xs">
                          {dep}
                        </span>
                      ))}
                    </div>
                ...
                {taskDef.blocks.length > 0 && (
                  <div>
                    <span className="text-gray-500 dark:text-gray-400 block mb-1">Blocks:</span>
                    <div className="flex flex-wrap gap-1.5">
                      {taskDef.blocks.map((block) => (
                        <span key={block} className="bg-gray-200 dark:bg-gray-800 text-gray-600 dark:text-gray-300 px-2 py-1 rounded font-mono text-xs">
                          {block}
                        </span>
                      ))}
                    </div>
          ...
          {taskDef.technical && (
            <div>
              <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Technical Details</h3>
              <div className="bg-gray-100 dark:bg-gray-900 rounded-lg p-4 space-y-4">
                {taskDef.technical.approach && (
                  <div>
                    <span className="text-gray-500 dark:text-gray-400 text-xs block mb-1">Approach</span>
                    <p className="text-sm text-gray-600 dark:text-gray-300">{taskDef.technical.approach}</p>
                  </div>
                )}
          ...
          {taskDef.criteria.length > 0 && (
            <div>
              <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Success Criteria</h3>
              <div className="bg-gray-100 dark:bg-gray-900 rounded-lg p-4">
                <ul className="space-y-2">
          ...
          <div>
            ...
            <div className="bg-gray-100 dark:bg-gray-900 rounded-lg p-4 space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="text-gray-600 dark:text-gray-300">{new Date(taskDef.created_at).toLocaleString()}</span>
              </div>
              {taskDef.queued_at && (
                <div className="flex justify-between">
                  <span className="text-gray-500 dark:text-gray-400">Queued</span>
                  <span className="text-gray-600 dark:text-gray-300">{new Date(taskDef.queued_at).toLocaleString()}</span>
                </div>
              )}
              {taskDef.started_at && (
                <div className="flex justify-between">
                  <span className="text-gray-500 dark:text-gray-400">Started</span>
                  <span className="text-gray-600 dark:text-gray-300">{new Date(taskDef.started_at).toLocaleString()}</span>
                </div>
              )}
              {taskDef.completed_at && (
                <div className="flex justify-between">
                  <span className="text-gray-500 dark:text-gray-400">Completed</span>
                  <span className="text-gray-600 dark:text-gray-300">{new Date(taskDef.completed_at).toLocaleString()}</span>
                </div>
              )}
            </div>
          </div>
              ...
              <div className={cn(
                ...
                {taskDef.result.summary && (
                  <div>
                    ...
                    <p className="text-gray-600 dark:text-gray-300 text-sm whitespace-pre-wrap">{taskDef.result.summary}</p>
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="text-center">
                    ...
                    <span className="text-lg font-medium text-gray-900 dark:text-white">{taskDef.result.iterations}</span>
                  </div>
                  <div className="text-center">
                    <span className="text-gray-500 dark:text-gray-400 text-xs block">Duration</span>
                    <span className="text-lg font-medium text-gray-900 dark:text-white">{Math.round(taskDef.result.duration / 1000)}s</span>
                  </div>
                  <div className="text-center">
                    <span className="text-gray-500 dark:text-gray-400 text-xs block">Tokens</span>
                    <span className="text-lg font-medium text-gray-900 dark:text-white">{taskDef.result.tokens.toLocaleString()}</span>
                  </div>
                  <div className="text-center">
                    <span className="text-gray-500 dark:text-gray-400 text-xs block">Status</span>
                    <span className={cn("text-lg font-medium", taskDef.result.success ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400")}>
                ...
                {taskDef.result.error && (
                  <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3">
                    ...
                    <p className="text-red-600 dark:text-red-300 text-sm">{taskDef.result.error}</p>
                  </div>
                )}
              </div>
            </div>
