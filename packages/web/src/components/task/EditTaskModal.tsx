    ...
    return (
      <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-8">
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-4">Loading task definition...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
          <div>
            <div className="flex items-center gap-3">
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white">{isYamlLinked ? "Edit Task Definition" : "Edit Task"}</h2>
              {isYamlLinked ? (
                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-forge-500/20 text-forge-500 dark:text-forge-400 border border-forge-500/30">
                  <FileCode className="h-3 w-3" />YAML Task
                </span>
              ) : (
                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-amber-500/20 text-amber-500 dark:text-amber-400 border border-amber-500/30">
                  <AlertTriangle className="h-3 w-3" />Raw Prompt
                </span>
              )}
            </div>
            {isYamlLinked && taskDefId && <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 font-mono">{taskDefId}</p>}
            {!isYamlLinked && (
              <p className="text-sm text-amber-500/80 dark:text-amber-400/80 mt-1 flex items-center gap-1">
          ...
          <button onClick={submit}
            ...
            className={cn("px-6 py-2 rounded-lg font-medium",
              (isYamlLinked ? (!title || updateTaskDef.isPending) : (!name || !prompt || updateTask.isPending))
                ? "bg-gray-300 dark:bg-gray-700 text-gray-500 cursor-not-allowed" : "bg-forge-500 text-white hover:bg-forge-600")}>
...
function YamlTaskFields(props: any) {
  ...
  return (
    <>
      <div className="grid grid-cols-2 gap-4">
        <div className="col-span-2">
          ...
          <input type="text" value={title} onChange={(e) => setTitle(e.target.value)}
            className="w-full bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2.5 text-gray-900 dark:text-white focus:border-forge-500 focus:outline-none" required />
        </div>
        <div className="col-span-2">
          <label className="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">Description</label>
          <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={4}
            className="w-full bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2.5 text-gray-900 dark:text-white resize-none focus:border-forge-500 focus:outline-none font-mono text-sm" required />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">Type</label>
          <select value={type} onChange={(e) => setType(e.target.value)}
            className="w-full bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2.5 text-gray-900 dark:text-white focus:border-forge-500 focus:outline-none">
            {TASK_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
          </select>
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">Complexity</label>
      ...
      <div>
        <label className="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2"><Target className="inline h-4 w-4 mr-1" />Goals</label>
        <div className="flex gap-2 mb-2">
          ...
          <button type="button" onClick={() => addToList(goals, setGoals, newGoal, setNewGoal)}
            className="px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-white rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600"><Plus className="h-5 w-5" /></button>
        </div>
        <div className="space-y-1">
          {goals.map((g: string, i: number) => (
            <div key={i} className="flex items-center justify-between bg-gray-100/50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2">
              <span className="text-gray-600 dark:text-gray-300 text-sm">{g}</span>
              <button type="button" onClick={() => removeFromList(goals, setGoals, i)} className="text-gray-500 hover:text-red-500 dark:hover:text-red-400"><Trash2 className="h-4 w-4" /></button>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
        <button type="button" onClick={() => setShowTechnical(!showTechnical)}
          className="w-full flex items-center justify-between p-4 bg-gray-100/50 dark:bg-gray-800/50 hover:bg-gray-200 dark:hover:bg-gray-800">
          <span className="flex items-center gap-2 text-sm font-medium text-gray-600 dark:text-gray-300"><FileCode className="h-4 w-4" />Technical Details</span>
          {showTechnical ? <ChevronUp className="h-4 w-4 text-gray-500 dark:text-gray-400" /> : <ChevronDown className="h-4 w-4 text-gray-500 dark:text-gray-400" />}
        </button>
        {showTechnical && (
          <div className="p-4 space-y-4 border-t border-gray-200 dark:border-gray-700">
            <div>
              <label className="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">Approach</label>
              <textarea value={approach} onChange={(e) => setApproach(e.target.value)} rows={6}
                className="w-full bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2.5 text-gray-900 dark:text-white resize-none focus:border-forge-500 focus:outline-none font-mono text-sm" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">Files to Create</label>
                <div className="flex gap-2 mb-2">
                  <input type="text" value={newFileCreate} onChange={(e) => setNewFileCreate(e.target.value)} placeholder="path/file.ts"
                    className="flex-1 bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-1.5 text-gray-900 dark:text-white text-sm font-mono focus:border-forge-500 focus:outline-none"
                  ...
                  <button type="button" onClick={() => addToList(filesToCreate, setFilesToCreate, newFileCreate, setNewFileCreate)}
                    className="px-3 py-1.5 bg-gray-200 dark:bg-gray-700 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600"><Plus className="h-4 w-4" /></button>
                </div>
                <div className="space-y-1 max-h-32 overflow-y-auto">
                  {filesToCreate.map((f: string, i: number) => (
                    <div key={i} className="flex items-center justify-between bg-green-500/10 border border-green-500/30 rounded px-2 py-1">
                      <code className="text-green-600 dark:text-green-400 text-xs">{f}</code>
                      <button type="button" onClick={() => removeFromList(filesToCreate, setFilesToCreate, i)} className="text-gray-500 hover:text-red-500 dark:hover:text-red-400"><X className="h-3 w-3" /></button>
                    </div>
                  ))}
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">Files to Modify</label>
                <div className="flex gap-2 mb-2">
                ...
                <div className="space-y-1 max-h-32 overflow-y-auto">
                  {filesToModify.map((f: string, i: number) => (
                    <div key={i} className="flex items-center justify-between bg-yellow-500/10 border border-yellow-500/30 rounded px-2 py-1">
                      <code className="text-yellow-600 dark:text-yellow-400 text-xs">{f}</code>
                      <button type="button" onClick={() => removeFromList(filesToModify, setFilesToModify, i)} className="text-gray-500 hover:text-red-500 dark:hover:text-red-400"><X className="h-3 w-3" /></button>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <div className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
        <button type="button" onClick={() => setShowExecution(!showExecution)}
          className="w-full flex items-center justify-between p-4 bg-gray-100/50 dark:bg-gray-800/50 hover:bg-gray-200 dark:hover:bg-gray-800">
          <span className="flex items-center gap-2 text-sm font-medium text-gray-600 dark:text-gray-300"><Zap className="h-4 w-4" />Execution Settings</span>
          {showExecution ? <ChevronUp className="h-4 w-4 text-gray-500 dark:text-gray-400" /> : <ChevronDown className="h-4 w-4 text-gray-500 dark:text-gray-400" />}
        </button>
        {showExecution && (
          <div className="p-4 grid grid-cols-2 gap-4 border-t border-gray-200 dark:border-gray-700">
            <div>
              <label className="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">Max Iterations</label>
              <input type="number" value={maxIterations} onChange={(e) => setMaxIterations(Number(e.target.value))} min={1} max={100}
                className="w-full bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2 text-gray-900 dark:text-white focus:border-forge-500 focus:outline-none" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">Checkpoint Every</label>
              <input type="number" value={checkpointEvery} onChange={(e) => setCheckpointEvery(Number(e.target.value))} min={1} max={50}
                className="w-full bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2 text-gray-900 dark:text-white focus:border-forge-500 focus:outline-none" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">On Stuck</label>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">Timeout (min)</label>
      ...
      <div className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
        <button type="button" onClick={() => setShowCriteria(!showCriteria)}
          className="w-full flex items-center justify-between p-4 bg-gray-100/50 dark:bg-gray-800/50 hover:bg-gray-200 dark:hover:bg-gray-800">
          <span className="flex items-center gap-2 text-sm font-medium text-gray-600 dark:text-gray-300"><Layers className="h-4 w-4" />Success Criteria ({criteria.length})</span>
          {showCriteria ? <ChevronUp className="h-4 w-4 text-gray-500 dark:text-gray-400" /> : <ChevronDown className="h-4 w-4 text-gray-500 dark:text-gray-400" />}
        </button>
        {showCriteria && (
          <div className="p-4 space-y-3 border-t border-gray-200 dark:border-gray-700">
              ...
              {CRITERION_PRESETS.map((p) => (
                <button key={p.label} type="button" onClick={() => addCriterion(p)}
                  className={cn("px-3 py-1.5 rounded-lg text-sm font-medium",
                    criteria.some((c: Criterion) => c.name === p.name) ? "bg-forge-500/20 text-forge-500 dark:text-forge-400 border border-forge-500/50" : "bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600")}>
                  {p.label}
                </button>
              ))}
            </div>
            <div className="flex gap-2">
              <input type="text" value={customCmd} onChange={(e) => setCustomCmd(e.target.value)} placeholder="Custom command..."
                className="flex-1 bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-gray-900 dark:text-white text-sm focus:border-forge-500 focus:outline-none"
                onKeyDown={(e) => e.key === "Enter" && (e.preventDefault(), addCustom())} />
              <button type="button" onClick={addCustom} className="px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-white rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600"><Plus className="h-4 w-4" /></button>
            </div>
            <div className="space-y-2">
              {criteria.map((c: Criterion, i: number) => (
                <div key={c.id || i} className="flex items-center justify-between bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2">
                  <div><span className="text-gray-900 dark:text-white">{c.name}</span><span className="text-gray-500 text-xs ml-2">({c.type})</span></div>
                  <button type="button" onClick={() => setCriteria(criteria.filter((_: Criterion, idx: number) => idx !== i))} className="text-gray-500 dark:text-gray-400 hover:text-red-500 dark:hover:text-red-400"><Trash2 className="h-4 w-4" /></button>
                </div>
              ))}
            </div>
          </div>
    ...
    <>
      <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-4">
        <div className="flex items-start gap-3">
          <FileText className="h-5 w-5 text-amber-500 dark:text-amber-400 mt-0.5" />
          <div>
            <h4 className="text-sm font-medium text-amber-600 dark:text-amber-300">Raw Prompt Task</h4>
      ...
      <div>
        ...
        <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g., Add auth"
          className="w-full bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2.5 text-gray-900 dark:text-white placeholder-gray-500 focus:border-forge-500 focus:outline-none" required />
      </div>
      ...
      <div>
        ...
        <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder="Describe the task..." rows={6}
          className="w-full bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2.5 text-gray-900 dark:text-white placeholder-gray-500 resize-none focus:border-forge-500 focus:outline-none" required />
      </div>
      ...
      <div>
        ...
        <div className="flex gap-2 mb-3">
          <input type="text" value={customCmd} onChange={(e) => setCustomCmd(e.target.value)} placeholder="Custom command"
            className="flex-1 bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2 text-gray-900 dark:text-white placeholder-gray-500 focus:border-forge-500 focus:outline-none"
            onKeyDown={(e) => e.key === "Enter" && (e.preventDefault(), addCustom())} />
          ...
          <div className="space-y-2">
            {criteria.map((c: Criterion) => (
              <div key={c.id} className="flex items-center justify-between bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2">
                ...
                <button type="button" onClick={() => setCriteria(criteria.filter((x: Criterion) => x.id !== c.id))} className="text-gray-500 dark:text-gray-400 hover:text-red-500 dark:hover:text-red-400"><Trash2 className="h-4 w-4" /></button>
              </div>
            ))}
          </div>
